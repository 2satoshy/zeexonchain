...
export const ConnectWalletModal: React.FC<ConnectWalletModalProps> = ({
  ...
  const [isClaimingModalAirdrop, setIsClaimingModalAirdrop] = useState(false);
  ...
  // Monitor and auto-claim 1,000 $ZIG airdrop for connected user
  useEffect(() => {
      ...
      .then(async (status) => {
        ...
        if (status.claimed) {
          ...
          try {
            ...
            if (claimRes.success && mounted) {
              setAirdropAllocated(true);
              setConnectionMessage('🎁 Welcome! 1,000 $ZIG Stablecoin + 400 SME stock tokens airdropped to your wallet!');
            }
          } catch (e) {
            console.warn('Airdrop auto-claim notice in modal:', e);
          }
        }
      })
      .catch((e) => console.warn('Airdrop status notice in modal:', e));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        {/* 1,000 $ZIG Airdrop Banner for Base, MetaMask, and Coinbase Users */}
        <div className="mx-5 sm:mx-6 mt-3 p-3 bg-gradient-to-r from-emerald-50 via-teal-50 to-blue-50 rounded-2xl border border-emerald-200/90 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-600 text-white flex items-center justify-center font-bold text-sm shadow-xs shrink-0">
              🎁
            </div>
            <div>
              <div className="text-xs font-black text-slate-900 flex items-center gap-1.5">
                <span>1,000 $ZIG Airdrop for Trades</span>
                <span className="text-[10px] font-extrabold bg-emerald-200 text-emerald-900 px-2 py-0.2 rounded-full">
                  $38.46 Free
                </span>
              </div>
              <p className="text-[11px] text-slate-600 mt-0.5 leading-snug">
                Sign in with Base, MetaMask, or Coinbase to get 1,000 ZIG + 400 SME stock tokens on Base Sepolia.
              </p>
            </div>
          </div>
        </div>
        ...
        {isAnyConnected && (
          <div className="mx-5 sm:mx-6 mt-3 p-3.5 bg-emerald-50 rounded-2xl border border-emerald-200 flex flex-col gap-2">
            ...
            <div className="pt-1 border-t border-emerald-200/60 flex items-center justify-between text-xs">
              <span className="text-emerald-900 font-medium text-[11px]">1,000 $ZIG Airdrop Status:</span>
              {airdropAllocated ? (
                <span className="text-[11px] font-bold text-emerald-700 flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                  <span>Allocated & Active for Trades</span>
                </span>
              ) : (
                <button
                  onClick={async () => {
                    if (!activeAddress) return;
                    setIsClaimingModalAirdrop(true);
                    try {
                      const res = await ApiService.claimAirdrop(activeAddress);
                      if (res.success) setAirdropAllocated(true);
                    } finally {
                      setIsClaimingModalAirdrop(false);
                    }
                  }}
                  disabled={isClaimingModalAirdrop}
                  className="text-[11px] font-bold bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-0.5 rounded-lg transition-colors cursor-pointer disabled:opacity-50"
                >
                  {isClaimingModalAirdrop ? 'Allocating...' : 'Claim 1,000 ZIG'}
                </button>
              )}
            </div>
          </div>
